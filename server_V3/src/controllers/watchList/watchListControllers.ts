...
export const deleteWatchList = (id: string) => WatchList.findOneAndUpdate({ userId: id, deletedAt:null }, { deletedAt: Date.now() });
export const searchWatchList = (userId:string, movieId:string)=> WatchList.findOne({movieId, userId, deletedAt:null})
